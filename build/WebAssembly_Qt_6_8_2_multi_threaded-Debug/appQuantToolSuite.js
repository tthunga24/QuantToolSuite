...
wasmBinaryFile = "appQuantToolSuite.wasm";
...
var PThread = {
 ...
 allocateUnusedWorker() {
  var worker;
  var pthreadMainJs = locateFile("appQuantToolSuite.worker.js");
...
function ___syscall_poll(fds, nfds, timeout) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 0, 1, fds, nfds, timeout);
 fds >>>= 0;
 try {
  var nonzero = 0;
...
function ___syscall_recvmsg(fd, message, flags, d1, d2, d3) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(26, 0, 1, fd, message, flags, d1, d2, d3);
 message >>>= 0;
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_renameat(olddirfd, oldpath, newdirfd, newpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(27, 0, 1, olddirfd, oldpath, newdirfd, newpath);
 oldpath >>>= 0;
 newpath >>>= 0;
 try {
...
function ___syscall_rmdir(path) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(28, 0, 1, path);
 path >>>= 0;
 try {
  path = SYSCALLS.getStr(path);
  FS.rmdir(path);
...
function ___syscall_sendmsg(fd, message, flags, d1, d2, d3) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(29, 0, 1, fd, message, flags, d1, d2, d3);
 message >>>= 0;
 d1 >>>= 0;
 d2 >>>= 0;
 try {
...
function ___syscall_socket(domain, type, protocol) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 0, 1, domain, type, protocol);
 try {
...
function ___syscall_stat64(path, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(31, 0, 1, path, buf);
 path >>>= 0;
 buf >>>= 0;
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_symlink(target, linkpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(32, 0, 1, target, linkpath);
 target >>>= 0;
 linkpath >>>= 0;
 try {
...
function ___syscall_truncate64(path, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(33, 0, 1, path, length);
 path >>>= 0;
 length = bigintToI53Checked(length);
 try {
  if (isNaN(length)) return 61;
...
function ___syscall_unlinkat(dirfd, path, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 0, 1, dirfd, path, flags);
 path >>>= 0;
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_utimensat(dirfd, path, times, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 0, 1, dirfd, path, times, flags);
 path >>>= 0;
 times >>>= 0;
 try {
  path = SYSCALLS.getStr(path);
...
function fetchXHR(fetch, onsuccess, onerror, onprogress, onreadystatechange) {
 ...
 function saveResponseAndStatus() {
  var ptr = 0;
  var ptrLen = 0;
  if (xhr.response && fetchAttrLoadToMemory && GROWABLE_HEAP_U32()[(((fetch) + (12)) >>> 2) >>> 0] === 0) {
  ...
  GROWABLE_HEAP_U32()[(((fetch) + (12)) >>> 2) >>> 0] = ptr;
  ...
  GROWABLE_HEAP_I16()[(((fetch) + (40)) >>> 1) >>> 0] = xhr.readyState;
  GROWABLE_HEAP_I16()[(((fetch) + (42)) >>> 1) >>> 0] = xhr.status;
...
function fetchCacheData(/** @type {IDBDatabase} */ db, fetch, data, onsuccess, onerror) {
 ...
 var fetch_attr = fetch + 112;
 var destinationPath = GROWABLE_HEAP_U32()[(((fetch_attr) + (64)) >>> 2) >>> 0];
 destinationPath ||= GROWABLE_HEAP_U32()[(((fetch) + (8)) >>> 2) >>> 0];
 ...
 try {
  ...
  putRequest.onsuccess = event => {
   GROWABLE_HEAP_I16()[(((fetch) + (40)) >>> 1) >>> 0] = 4;
   GROWABLE_HEAP_I16()[(((fetch) + (42)) >>> 1) >>> 0] = 200;
   stringToUTF8("OK", fetch + 44, 64);
   onsuccess(fetch, 0, destinationPathStr);
  };
  putRequest.onerror = error => {
   GROWABLE_HEAP_I16()[(((fetch) + (40)) >>> 1) >>> 0] = 4;
   GROWABLE_HEAP_I16()[(((fetch) + (42)) >>> 1) >>> 0] = 413;
...
function fetchLoadCachedData(db, fetch, onsuccess, onerror) {
 ...
 var fetch_attr = fetch + 112;
 var path = GROWABLE_HEAP_U32()[(((fetch_attr) + (64)) >>> 2) >>> 0];
 path ||= GROWABLE_HEAP_U32()[(((fetch) + (8)) >>> 2) >>> 0];
 ...
 try {
  ...
  getRequest.onsuccess = event => {
   if (event.target.result) {
    ...
    GROWABLE_HEAP_I16()[(((fetch) + (40)) >>> 1) >>> 0] = 4;
    GROWABLE_HEAP_I16()[(((fetch) + (42)) >>> 1) >>> 0] = 404;
...
function fetchDeleteCachedData(db, fetch, onsuccess, onerror) {
 ...
 try {
  ...
  request.onsuccess = event => {
   var value = event.target.result;
   GROWABLE_HEAP_U32()[(((fetch) + (12)) >>> 2) >>> 0] = 0;
...
function _emscripten_start_fetch(fetch, successcb, errorcb, progresscb, readystatechangecb) {
 ...
 runtimeKeepalivePush();
 var fetch_attr = fetch + 112;
 var onsuccess = GROWABLE_HEAP_U32()[(((fetch_attr) + (36)) >>> 2) >>> 0];
 var onerror = GROWABLE_HEAP_U32()[(((fetch_attr) + (40)) >>> 2) >>> 0];
 var onprogress = GROWABLE_HEAP_U32()[(((fetch_attr) + (44)) >>> 2) >>> 0];
 var onreadystatechange = GROWABLE_HEAP_U32()[(((fetch_attr) + (48)) >>> 2) >>> 0];
 var fetchAttributes = GROWABLE_HEAP_U32()[(((fetch_attr) + (52)) >>> 2) >>> 0];
 var fetchAttrSynchronous = !!(fetchAttributes & 64);
...
var ___start_em_js = Module["___start_em_js"] = 12448840;

var ___stop_em_js = Module["___stop_em_js"] = 12449930;
...
var missingLibrarySymbols = [ "writeI53ToI64Clamped", "writeI53ToI64Signaling", "writeI53ToU64Clamped", "writeI53ToU64Signaling", "convertI32PairToI53Checked", "convertPCtoSourceLocation", "readEmAsmArgs", "listenOnce", "autoResumeAudioContext", "getDynCaller", "dynCall", "asmjsMangle", "getNativeTypeSize", "STACK_SIZE", "STACK_ALIGN", "POINTER_SIZE", "ASSERTIONS", "getCFunc", "ccall", "cwrap", "uleb128Encode", "sigToWasmTypes", "generateFuncType", "convertJsFunctionToWasm", "getEmptyTableSlot", "updateTableMap", "getFunctionAddress", "addFunction", "removeFunction", "intArrayToString", "AsciiToString", "registerKeyEventCallback", "registerMouseEventCallback", "registerFocusEventCallback", "fillDeviceOrientationEventData", "registerDeviceOrientationEventCallback", "fillDeviceMotionEventData", "registerDeviceMotionEventCallback", "screenOrientation", "fillOrientationChangeEventData", "registerOrientationChangeEventCallback", "fillFullscreenChangeEventData", "registerFullscreenChangeEventCallback", "JSEvents_requestFullscreen", "JSEvents_resizeCanvasForFullscreen", "registerRestoreOldStyle", "hideEverythingExceptGivenElement", "restoreHiddenElements", "setLetterbox", "softFullscreenResizeWebGLRenderTarget", "doRequestFullscreen", "fillPointerlockChangeEventData", "registerPointerlockChangeEventCallback", "registerPointerlockErrorEventCallback", "requestPointerLock", "fillVisibilityChangeEventData", "registerVisibilityChangeEventCallback", "registerTouchEventCallback", "fillGamepadEventData", "registerGamepadEventCallback", "registerBeforeUnloadEventCallback", "fillBatteryEventData", "battery", "registerBatteryEventCallback", "setCanvasElementSizeCallingThread", "setCanvasElementSizeMainThread", "setCanvasElementSize", "getCanvasSizeCallingThread", "getCanvasSizeMainThread", "getCanvasElementSize", "stackTrace", "checkWasiClock", "wasiRightsToMuslOFlags", "wasiOFlagsToMuslOFlags", "createDyncallWrapper", "setImmediateWrapped", "clearImmediateWrapped", "polyfillSetImmediate", "getPromise", "makePromise", "idsToPromises", "makePromiseCallback", "Browser_asyncPrepareDataCounter", "FS_unlink", "FS_mkdirTree", "writeGLArray", "emscripten_webgl_destroy_context_before_on_calling_thread", "registerWebGlEventCallback", "runAndAbortIfError", "ALLOC_NORMAL", "ALLOC_STACK", "allocate", "writeStringToMemory", "writeAsciiToMemory", "setErrNo", "demangle", "getFunctionArgsName", "createJsInvokerSignature", "registerInheritedInstance", "unregisterInheritedInstance", "enumReadValueFromPointer", "validateThis" ];
...
if (typeof exports === 'object' && typeof module === 'object')
  module.exports = appQuantToolSuite_entry;
else if (typeof define === 'function' && define['amd'])
  define([], () => appQuantToolSuite_entry);
